...
async def main() -> None:
    print("Phase 1: Sync stock instruments to PostgreSQL...")
    result = await sync_stock_instruments_to_pg()
    ...
    while current <= end_date:
        ...
        print(f"\n  [{day_label}] Fetching...")
        try:
            ob_result = await backfill_stock_order_books(
                start_date=current,
                end_date=current,
            )
            ...
            tr_result = await backfill_stock_trades(
                start_date=current,
                end_date=current,
            )
