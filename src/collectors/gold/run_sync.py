...
from src.collectors.gold.order_book_fetcher import backfill_gold_order_books
...
async def main() -> None:
    print("Phase 1: Sync gold instruments to PostgreSQL...")
    result = await sync_gold_instruments_to_pg()
    ...
    print(f"\nPhase 2: Backfill gold order books + trades for {DAYS_TO_FETCH} days...")
    ...
    while current <= end_date:
        ...
        print(f"\n  [{day_label}] Fetching...")
        try:
            ob_result = await backfill_gold_order_books(
                start_date=current,
                end_date=current,
            )
            ...
            tr_result = await backfill_gold_trades(
                start_date=current,
                end_date=current,
            )
