from src.db.clickhouse.schema import OPTION_ORDER_BOOK_TABLE
...
def upgrade(client):
    client.command(_OPTION_ORDER_BOOK_DDL)


def downgrade(client):
    client.command(f"DROP TABLE IF EXISTS `{OPTION_ORDER_BOOK_TABLE}`")
