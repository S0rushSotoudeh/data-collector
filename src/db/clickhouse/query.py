...
_OPT_OB_COLUMNS = OPTION_ORDER_BOOK_COLUMNS
_OPT_TR_COLUMNS = OPTION_TRADES_COLUMNS
_STK_OB_COLUMNS = STOCK_ORDER_BOOK_COLUMNS
_STK_TR_COLUMNS = STOCK_TRADES_COLUMNS
...
def _row_to_dict_opt_ob(row: tuple[Any, ...]) -> dict[str, Any]:
    d = dict(zip(_OPT_OB_COLUMNS, row))
    d["bid_price"] = price_from_storage(d["bid_price"])
    d["ask_price"] = price_from_storage(d["ask_price"])
    return d


def _row_to_dict_opt_tr(row: tuple[Any, ...]) -> dict[str, Any]:
    d = dict(zip(_OPT_TR_COLUMNS, row))
    d["price"] = price_from_storage(d["price"])
    d["value"] = price_from_storage(d["value"])
    return d


async def get_option_latest_order_book(
    instrument_code: str,
    trade_date: date,
) -> list[dict[str, Any]]:
    client = await get_async_client()
    q = (
        f"SELECT * FROM `{OPTION_ORDER_BOOK_TABLE}` FINAL "
        f"WHERE instrument_code = {{code:String}} AND trade_date = {{dt:Date}} "
        f"ORDER BY trade_time DESC, depth_level ASC "
        f"LIMIT 5"
    )
    rows = (await client.query(q, parameters={"code": instrument_code, "dt": trade_date})).result_rows
    return [_row_to_dict_opt_ob(r) for r in rows]


async def get_option_order_book_history(
    instrument_code: str,
    trade_date: date,
) -> list[dict[str, Any]]:
    client = await get_async_client()
...
async def get_option_latest_order_books(
    limit: int = 10,
) -> list[dict[str, Any]]:
    client = await get_async_client()
    q = (
...
async def get_option_trade_history(
    ...
    client = await get_async_client()
    q = (
        f"SELECT * FROM `{OPTION_TRADES_TABLE}` FINAL "
        f"WHERE instrument_code = {{code:String}} AND trade_date = {{dt:Date}} "
        f"ORDER BY trade_time ASC "
        f"LIMIT {{lim:UInt16}}"
    )
    rows = (await client.query(q, parameters={"code": instrument_code, "dt": trade_date, "lim": limit})).result_rows
    return [_row_to_dict_opt_tr(r) for r in rows]


async def get_option_vwap(
    instrument_code: str,
    trade_date: date,
) -> dict[str, Any] | None:
    client = await get_async_client()
    q = (
        ...
        f"    sum(value) AS total_value, "
        f"    count() AS trade_count "
        f"FROM `{OPTION_TRADES_TABLE}` FINAL "
        f"WHERE instrument_code = {{code:String}} "
        f"  AND trade_date = {{dt:Date}} "
        f"  AND is_canceled = 0"
    )
...
async def get_option_daily_spread(
    instrument_code: str,
    trade_date: date,
) -> dict[str, Any] | None:
    client = await get_async_client()
    q = (
        ...
        f"    max(ask_price - bid_price) AS max_spread, "
        f"    avg(ask_price - bid_price) AS avg_spread "
        f"FROM `{OPTION_ORDER_BOOK_TABLE}` FINAL "
        f"WHERE instrument_code = {{code:String}} "
        f"  AND trade_date = {{dt:Date}} "
        f"  AND depth_level = 1 "
        f"  AND bid_price > 0 AND ask_price > 0"
...
async def get_option_latest_trades(
    limit: int = 10,
) -> list[dict[str, Any]]:
    client = await get_async_client()
    q = (
...
async def get_option_order_book_paginated(
    instrument_code: str | None = None,
    trade_date: date | None = None,
    depth_level: int | None = None,
    data_source: str | None = None,
    ...
    client = await get_async_client()
    specs = _opt_ob_filters(instrument_code, trade_date, depth_level, data_source)
    where, params = _build_where(specs)
    params["lim"] = limit
    params["off"] = offset
    q = (
        f"SELECT * FROM `{OPTION_ORDER_BOOK_TABLE}` FINAL {where} "
        f"ORDER BY trade_date DESC, trade_time DESC, depth_level ASC "
        f"LIMIT {{lim:UInt32}} OFFSET {{off:UInt32}}"
    )
    rows = (await client.query(q, parameters=params)).result_rows
...
async def count_option_order_book(
    instrument_code: str | None = None,
    trade_date: date | None = None,
    depth_level: int | None = None,
    data_source: str | None = None,
    ...
    specs = _opt_ob_filters(instrument_code, trade_date, depth_level, data_source)
    where, params = _build_where(specs)
    q = f"SELECT count() FROM `{OPTION_ORDER_BOOK_TABLE}` FINAL {where}"
    rows = (await client.query(q, parameters=params)).result_rows
    return int(rows[0][0]) if rows else 0


async def get_option_trades_paginated(
    instrument_code: str | None = None,
    trade_date: date | None = None,
    min_price: int | None = None,
    max_price: int | None = None,
    ...
    client = await get_async_client()
    specs = _opt_tr_filters(instrument_code, trade_date, min_price, max_price, is_canceled, data_source)
    where, params = _build_where(specs)
    params["lim"] = limit
    params["off"] = offset
    q = (
        f"SELECT * FROM `{OPTION_TRADES_TABLE}` FINAL {where} "
        f"ORDER BY trade_date DESC, trade_time DESC "
        f"LIMIT {{lim:UInt32}} OFFSET {{off:UInt32}}"
    )
    rows = (await client.query(q, parameters=params)).result_rows
...
async def count_option_trades(
    ...
    specs = _opt_tr_filters(instrument_code, trade_date, min_price, max_price, is_canceled, data_source)
    where, params = _build_where(specs)
    q = f"SELECT count() FROM `{OPTION_TRADES_TABLE}` FINAL {where}"
    rows = (await client.query(q, parameters=params)).result_rows
    return int(rows[0][0]) if rows else 0


async def get_option_trades_intraday(
    instrument_code: str,
    trade_date: date,
    from_time: int | None = None,
    to_time: int | None = None,
...
async def get_option_trades_daily(
    instrument_code: str,
    from_date: date,
    to_date: date,
) -> list[dict[str, Any]]:
...
async def get_option_trades_ranking(from_date: date, to_date: date) -> list[dict[str, Any]]:
    client = await get_async_client()
    q = (
        f"SELECT instrument_code, sum(value) AS val "
        f"FROM `{OPTION_TRADES_TABLE}` FINAL "
...
def _row_to_dict_stk_ob(row: tuple[Any, ...]) -> dict[str, Any]:
    d = dict(zip(_STK_OB_COLUMNS, row))
    d["bid_price"] = price_from_storage(d["bid_price"])
    d["ask_price"] = price_from_storage(d["ask_price"])
    return d


def _row_to_dict_stk_tr(row: tuple[Any, ...]) -> dict[str, Any]:
    d = dict(zip(_STK_TR_COLUMNS, row))
    d["price"] = price_from_storage(d["price"])
    d["value"] = price_from_storage(d["value"])
    return d


async def get_stock_latest_order_book(
    instrument_code: str,
    trade_date: date,
) -> list[dict[str, Any]]:
    client = await get_async_client()
    q = (
        f"SELECT * FROM `{STOCK_ORDER_BOOK_TABLE}` FINAL "
        f"WHERE instrument_code = {{code:String}} AND trade_date = {{dt:Date}} "
        f"ORDER BY trade_time DESC, depth_level ASC "
        f"LIMIT 5"
    )
    rows = (await client.query(q, parameters={"code": instrument_code, "dt": trade_date})).result_rows
    return [_row_to_dict_stk_ob(r) for r in rows]


async def get_stock_order_book_history(
    instrument_code: str,
    trade_date: date,
) -> list[dict[str, Any]]:
    client = await get_async_client()
...
async def get_stock_latest_order_books(
    limit: int = 10,
) -> list[dict[str, Any]]:
    client = await get_async_client()
    q = (
...
async def get_stock_trade_history(
    instrument_code: str,
    trade_date: date,
    limit: int = 500,
) -> list[dict[str, Any]]:
    client = await get_async_client()
    q = (
        f"SELECT * FROM `{STOCK_TRADES_TABLE}` FINAL "
        f"WHERE instrument_code = {{code:String}} AND trade_date = {{dt:Date}} "
        f"ORDER BY trade_time ASC "
        f"LIMIT {{lim:UInt16}}"
    )
    rows = (await client.query(q, parameters={"code": instrument_code, "dt": trade_date, "lim": limit})).result_rows
    return [_row_to_dict_stk_tr(r) for r in rows]


async def get_stock_vwap(
    instrument_code: str,
    trade_date: date,
) -> dict[str, Any] | None:
    client = await get_async_client()
...
async def get_stock_daily_spread(
    instrument_code: str,
    trade_date: date,
) -> dict[str, Any] | None:
    client = await get_async_client()
...
async def get_stock_latest_trades(
    limit: int = 10,
) -> list[dict[str, Any]]:
    client = await get_async_client()
    q = (
...
async def get_stock_order_book_paginated(
    instrument_code: str | None = None,
    trade_date: date | None = None,
    depth_level: int | None = None,
    data_source: str | None = None,
    ...
    client = await get_async_client()
    specs = _stk_ob_filters(instrument_code, trade_date, depth_level, data_source)
    where, params = _build_where(specs)
    params["lim"] = limit
    params["off"] = offset
    q = (
        f"SELECT * FROM `{STOCK_ORDER_BOOK_TABLE}` FINAL {where} "
        f"ORDER BY trade_date DESC, trade_time DESC, depth_level ASC "
        f"LIMIT {{lim:UInt32}} OFFSET {{off:UInt32}}"
    )
    rows = (await client.query(q, parameters=params)).result_rows
...
async def count_stock_order_book(
    instrument_code: str | None = None,
    trade_date: date | None = None,
    depth_level: int | None = None,
    data_source: str | None = None,
    ...
    specs = _stk_ob_filters(instrument_code, trade_date, depth_level, data_source)
    where, params = _build_where(specs)
    q = f"SELECT count() FROM `{STOCK_ORDER_BOOK_TABLE}` FINAL {where}"
    rows = (await client.query(q, parameters=params)).result_rows
    return int(rows[0][0]) if rows else 0


async def get_stock_trades_paginated(
    instrument_code: str | None = None,
    trade_date: date | None = None,
    min_price: int | None = None,
    max_price: int | None = None,
    ...
    client = await get_async_client()
    specs = _stk_tr_filters(instrument_code, trade_date, min_price, max_price, is_canceled, data_source)
    where, params = _build_where(specs)
    params["lim"] = limit
    params["off"] = offset
    q = (
        f"SELECT * FROM `{STOCK_TRADES_TABLE}` FINAL {where} "
        f"ORDER BY trade_date DESC, trade_time DESC "
        f"LIMIT {{lim:UInt32}} OFFSET {{off:UInt32}}"
    )
    rows = (await client.query(q, parameters=params)).result_rows
...
async def count_stock_trades(
    ...
    specs = _stk_tr_filters(instrument_code, trade_date, min_price, max_price, is_canceled, data_source)
    where, params = _build_where(specs)
    q = f"SELECT count() FROM `{STOCK_TRADES_TABLE}` FINAL {where}"
    rows = (await client.query(q, parameters=params)).result_rows
    return int(rows[0][0]) if rows else 0


async def get_stock_trades_intraday(
    instrument_code: str,
    trade_date: date,
    from_time: int | None = None,
    to_time: int | None = None,
...
async def get_stock_trades_daily(
    instrument_code: str,
    from_date: date,
    to_date: date,
) -> list[dict[str, Any]]:
...
async def get_stock_trades_ranking(from_date: date, to_date: date) -> list[dict[str, Any]]:
    client = await get_async_client()
    q = (
        f"SELECT instrument_code, sum(value) AS val "
        f"FROM `{STOCK_TRADES_TABLE}` FINAL "
